...
NEXT_PUBLIC_CDN_BASE_URL=https://cdn.moni.my     # 프로필 이미지 CDN (next.config.ts remotePatterns)
...
사용자
  GET    /api/v1/users/me                              # response: profile, oauthProvider, integrated 포함
  PATCH  /api/v1/users/me                              # name, nickname, phone 수정
  POST   /api/v1/users/me/password                     # 비밀번호 변경 (currentPassword 필요)
  POST   /api/v1/users/me/integrate                    # OAuth 유저 통합 회원 전환 (비밀번호 설정)
  DELETE /api/v1/users/me                              # body: { password } — 비밀번호 있는 유저만 필수
  GET    /api/v1/users/me/profile/presigned-url        # query: extension(jpg/jpeg/png/webp) → { presignedUrl, s3Url }
  PATCH  /api/v1/users/me/profile                      # body: { profile } — 이모지 or S3 URL
  POST   /api/v1/users/me/tendency
  GET    /api/v1/users/me/tendency
  POST   /api/v1/users/me/interests
  GET    /api/v1/users/me/watchlist
  POST   /api/v1/users/me/watchlist/{ticker}           # ticker: 영문/숫자 6자리
...
자산 (trade-service 라우팅)
  GET  /api/v1/assets                       # 총자산 요약 (totalAsset, cashBalance, stockEvaluationAmount, totalProfitLoss, totalReturnRate)
  GET  /api/v1/assets/holdings             # 보유 종목 페이지네이션 (currentPrice 포함)
